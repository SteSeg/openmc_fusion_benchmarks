...
def _validate_tally_consistency(spec_tally: dict, observed_tally: dict) -> tuple[bool, list[str]]:
    ...
    if len(spec_filters) == len(observed_non_particle):
        for i, (spec_filter, observed_axis) in enumerate(zip(spec_filters, observed_non_particle)):
            ftype = _normalize_filter_type(spec_filter.get("type", ""))
            if ftype == "energy":
                expected_closure = spec_filter.get("closure", "[low, high)")
                observed_closure = observed_axis.get("closure", "[low, high)")
                if expected_closure != observed_closure:
                    issues.append(
                        "energy closure mismatch at index "
                        f"{i}: expected {expected_closure}, observed {observed_closure}"
                    )
