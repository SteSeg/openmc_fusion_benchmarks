...
def minimal_tmc(models: Iterable[openmc.Model], cross_sections: Iterable[Union[str, Path]]):
    ...
    for i, model in models:
        ...
        for j, cs in cross_sections:
            openmc.config['cross_sections'] = cs

            # Run model with the current cross sections
            model.run(cwd=f"model_{i}/cs_{j}")

    return


def get_tmc_results(tmc_results: Union[str, Path]) -> xr.Dataset:
    """Load TMC results from a file and return as an xarray Dataset.
    Currently the 'tmc_results' argument is a list of paths to the 
    openmc statepoint files generated by the TMC simulation."""
    ...
    tmc_results = xr.open_dataset(tmc_results, engine='h5netcdf')
    return tmc_results
